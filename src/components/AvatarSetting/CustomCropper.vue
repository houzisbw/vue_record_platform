  ...
  <div class="cropper-modal">
    ...
    <div class="modal">
      ...
      <div class="modal-footer">
        ...
        <a class="modal-btn btn-confirm"
           @click="saveProfileAndShow"
           :class="{'disabled':!isResetButtonShow||disableButton}">
          {{disableButton?'更新中...':'更新'}}
        </a>
      </div>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    mounted () {
      ...
      //当前头像
      var currentProfile = this.$store.getters.getUserProfileImg || this.defaultUserAvatarUrl;
    ...
    methods: {
      ...
      saveProfileAndShow(){
          ...
          //生产环境跨域不能像上面那样写
          this.axios.post('/avatarUpload',data).then((resp)=>{
          	  //注意这里有2个data，第一个是axios的data，第二个是图床网站的data属性
              //获取图片的url,存入数据库
              var imgUrl = resp.data.data.url;
              this.axios.post(api.updateUserAvatar,{url:imgUrl}).then((resp)=>{
                  ...
                  setTimeout(()=>{
                    ...
                    //更新vuex
                    this.$store.commit('updateUserProfile',imgUrl)
                  },1000)
              })
...
<style scoped type="text/less" lang="less">
  ...
  .label-select{
    ...
    height:202px;
    border-radius: 4px;
    background: url('./../../assets/images/icon/add-image.png') center center / 60px 60px no-repeat #e7e7e7;
  }
  /*按钮禁止*/
  .disabled{
